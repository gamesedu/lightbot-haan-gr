//20150702 v003 Mod by jon  - increase height,set map data to normal JSON object (like the lightbot game s maps.txt)
//20150711 v008   Mod by jon - open on the fly
...
// ########  JON :  show created code windows
    that.retrieveMap = function() {
	
      var fullMap = {
        "direction": 0,
		"position": {"x": 0, "y": 0},
        "map": that.map.getJONRotatedMapData(),
        "medals": {
          "gold": 3,
          "silver": 4,
          "bronze": 5
...
// ########  JON :  show created code windows
    that.retrieveAndOpenMap = function() {
...
(function() {
 ...
 //data.map = rotateArrayRight(data.map); //added by jon 20150701
  mapData = data.map;
      ...
      for (var i = 0; i < levelSize.x; i++) {
		...
		// ############## jon : charAt(1) takes 1b ,2l etc and reads light or blank ##########
   ...
   //   that.rotateLeft();
	...
	// ############## jon : charAt(0) takes height from 1b ,2l etc ##########

    function getHeight(x, y) {
      return parseInt(mapData[x][y].h);
    }

    function setHeight(x, y, height) {
     // mapData[x][y].h = height + mapData[x][y].t;
	  mapData[x][y].h = height;

    }

    function getType(x, y) {
      return mapData[x][y].t;
    }

    function setType(x, y, type) {
      //mapData[x][y].t = mapData[x][y].h + type;
	   mapData[x][y].t =  type;
      if (type === 'b') {
        mapRef[x][y] = box;
      } else if (type === 'l') {
...
//    function
      ...
      if (levelSize.x < width) {
        for (var i = levelSize.x; i < width; i++) {
          ...
          for (var j = 0; j < levelSize.y; j++) {
            mapData[i][j] = "1b"; //orig
			mapData[i][j] = {"h":1, "t":"b"}; //########## jon
		    //mapData[i][j].h = "1"; mapData[i][j].t = "b";	  //########## jon
            mapRef[i][j] = box;
          }
        }
      } else {
    ...
    that.getJONRotatedMapData = function() { //############ JON !!!! ADDED 20150701 so that maps.txt will dispay correct rotated map
	
      return rotateArrayRight(rotateArrayRight(rotateArrayRight(mapData)));
    };	
